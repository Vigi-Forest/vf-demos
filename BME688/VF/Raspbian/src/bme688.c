...
/*
    //printf("BME68X is initialized\n");
    if (bme.variant_id == BME68X_VARIANT_GAS_LOW) {
      my_printf("Variant = BME68X_VARIANT_GAS_LOW\n");
    } 
    else{
      my_printf("Variant = BME68X_VARIANT_GAS_HIGH\n");
    } 
*/
  } 
  else{
...
//   printf("os_hum = %#0x, os_pres = %#0x, os_temp = %#0x, filter = %#0x, odr = %#0x\n", conf.os_hum, conf.os_pres, conf.os_temp, conf.filter, conf.odr);
  ...
  if (n_fields) {
    ...
    if (json_display)
      ...
      printf("Temp %.2f C Prs %.2f hPa Hum %.2f %% GSR %.2f kOhms IAQ %g eCO2 %g ppm\n",
	     ...
	     data.pressure/100,
	     data.humidity,
	     data.gas_resistance/1000, self.iaqScore, self.eCO2Value);
    }
#else
  ...
  if (i2c_close(&linux_device_handle)) {
    fprintf(stderr, "i2c_close() error\n");
    exit (1);
  }
